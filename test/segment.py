...
writer.writerow(["Image", "Duckweed %", "Water %"])
...
for filename in os.listdir(INPUT_FOLDER):
    ...
    if filename.endswith(".jpg") or filename.endswith(".png"):
        ...
        upper_water = np.array([180, 255, 80])
        ...
        total_pixels = duckweed_pixels + water_pixels

        if total_pixels > 0:

            duckweed_percent = round(
                (duckweed_pixels / total_pixels) * 100,
                2
            )
        ...
        result[duckweed_mask > 0] = [0, 255, 0]

        # Blue overlay = water
        result[water_mask > 0] = [255, 0, 0]

        # Detect and label rings
        rings = detect_rings(image)
        ...
        print(filename)
        print("Duckweed:", duckweed_percent, "%")
        print("Water:", water_percent, "%")
        print("Rings Detected:", len(rings))
        print("---------------------")
